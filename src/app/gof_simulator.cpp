...
static void show_usage(std::string name) {
  std::cerr
      ...
      << "\t-g,--generation \tSpecify the maximum number of generation, and "
         "the default number is 10\n"
      << "\t-s,--size \t\tSpecify the size of grid, please input two integers "
         "as row and colnum\n"
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if ((arg == "-h") || (arg == "--help")) {
      ...
      {
        filename = argv[++i]; // Increment 'i' so we don't get the argument as
                              // the next argv[i].
      } else { // Uh-oh, there was no argument to the destination option.
        std::cerr << "--file option requires one file path as argument."
                  << std::endl;
      ...
      if (i + 1 < argc) {
        ...
        generation = std::atoi(argv[++i]); // convert char to int

      } else {
        std::cerr << "--generation option requires one integer as argument."
                  << std::endl;
  ...
  if (!filename.empty()) { // input a file path
