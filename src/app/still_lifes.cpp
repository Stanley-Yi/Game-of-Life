...
static void show_usage(std::string name)
{
    std::cerr << "Usage: " << name << " <option(s)> SOURCES"
              ...
              << "\t-i,--iteration \tSpecify the maximum number of iterations, and the default number is 10\n"
              ...
              << "\t-c,--condition \t\tSpecify the number of different initial conditions need to try, and the default number is 3\n"
              << std::endl;
}
    ...
    for (int k = 0; k < iteration; k ++)
    {
        ...
        for (int i = 0; i < r - 3; i ++)
        {
            for (int j = 0; j < c - 3; j ++)
            {
                ...
                {
                    std::cout << "Program found a still life, the grid of last and current and the pattern is:" << std::endl;
                    last -> printGrid();
                    grid.printGrid();
...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++) 
    {
        ...
        else if ((arg == "-i") || (arg == "--iteration")) 
        {
            ...
            {
                std::cerr << "--iteration option requires one integer as argument." << std::endl;
        ...
        else if ((arg == "-c") || (arg == "--condition")) 
        {
            ...
            {
                std::cerr << "--condition option requires one integer as argument." << std::endl;
