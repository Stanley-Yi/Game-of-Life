...
LifeGame::LifeGame(uint N, uint M, uint num)
    : grid(GridStatus(N, M, num)), generation(0) {}

LifeGame::LifeGame(std::string filename)
    : grid(GridStatus(filename)), generation(0) {}
...
void LifeGame::takeStep() {
  ...
  GridStatus *temp = new GridStatus(grid); // temporary variable to store grid
  ...
  for (int i = 0; i < row; i++) {
    for (int j = 0; j < col; j++) {
      ...
      // A dead cell with exactly three live neighbours should become a live
      // cell
      ...
      // A live cell with less than two or more than three live neighbours
      // should die
      else if (status == 'o' & (neighbour<2 | neighbour> 3)) {
        temp->setStatus('-', i, j);
      }
...
char LifeGame::getStatus(uint x, uint y) { return grid.getStatus(x, y); }
