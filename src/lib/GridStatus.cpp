...
GridStatus::GridStatus(uint N, uint M, uint num) : row(N), col(M) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    this->setStatus('o', location / col,
                    location >= col ? location % col : location);
  }
}
    ...
    if ((std::string)&s == "\r\n" | s == '\n' | s == '\r') {
      ...
      if (s != '-' and s != 'o') {
        throw std::invalid_argument(
            "The content of file should only be - or o.");
      }
...
char &GridStatus::operator()(uint x, uint y) { return grid.at(x).at(y); }

void GridStatus::setStatus(char s, uint x, uint y) { (*this)(x, y) = s; }

char GridStatus::getStatus(uint x, uint y) { return (*this)(x, y); }
